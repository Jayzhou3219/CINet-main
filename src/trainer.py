...
class trainer(object):
    ...
    def _initialize_network(self, model_name: str):
        ...
        """
        if model_name == 'ZJL':
            from src.models.zjl_serial import ZJL_GOAL
            #from src.models.zjl import ZJL_GOAL
            net = ZJL_GOAL(self.args, self.device)
        elif model_name == 'ZJL_GOAL':
            from src.models.zjl_goal import ZJL_GOAL
            #from src.models.CINetr import ZJL_GOAL
            #from src.models.CINetv import ZJL_GOAL
            net = ZJL_GOAL(self.args, self.device)
        else:
        ...
        if self.args.load_checkpoint is not None:
            ...
            with open(self.log_curve_file, 'w') as f:
                f.write("epoch,learning_rate,"
                        "train_ADE_world,train_FDE_world,"
                        "valid_ADE_world,valid_FDE_world," +
    ...
    def train(self):
        ...
        # Set scheduler
        self.scheduler = self._set_scheduler(self.args.scheduler)  # sched_setscheduler()函数用以修改线程的调度策略以及调度参数
    ...
    def _train_loop(self, start_epoch, end_epoch):
        ...
        valid_metrics = {"valid_ADE_world": 0, "valid_FDE_world": 0}
        ...
        if wandb.run is None and self.args.use_wandb:
            wandb.init(settings=wandb.Settings(start_method="thread"),
                       project="GoalSAR", config=self.args,
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if epoch >= self.args.start_validation and \
                ...
                valid_losses, valid_metrics = self._evaluate_epoch(epoch, mode='valid')
            ...
            with open(self.log_curve_file, 'a') as f:
                f.write(','.join(str(m) for m in [
                    epoch, learning_rate,
                    train_metrics["train_ADE_world"],
                    train_metrics["train_FDE_world"],
                    valid_metrics["valid_ADE_world"],
                    valid_metrics["valid_FDE_world"]] +
                                 [train_losses[loss_name] for loss_name in sorted(train_losses)]) + '\n')
    ...
    def _train_epoch(self, epoch):
        ...
        # Progress bar
        train_bar = tqdm(self.data_loaders['train'], ascii=True, ncols=100, desc=f'Epoch {epoch}. Train batches')
        num_train_batches = len(self.data_loaders['train'])

        for batch_data, batch_id in train_bar:

            inputs, ground_truth, seq_list = self.net.prepare_inputs(batch_data, batch_id)

            del batch_data

            self.optimizer.zero_grad()  # sets grads to zero

            all_outputs, all_aux_outputs = self.net.forward(inputs, ground_truth, if_test=False)  # Forward network

            loss_mask = self.net.compute_loss_mask(              # compute loss_mask
                seq_list, self.args.obs_length).to(self.device)  # 如果行人i在时间t存在,则loss_mask[t，i]=1

            metric_mask = compute_metric_mask(seq_list)  # compute metric_mask  # 如果行人i在观察和预测期间完全存在，则metric_mask[i]=1

            losses = self.net.compute_model_losses(      # compute model losses
                all_outputs, ground_truth, loss_mask, inputs, all_aux_outputs)

            loss = torch.zeros(1).to(self.device)        # overall loss
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.net.parameters(), self.args.clip)  # 梯度裁剪
    ...
    def _evaluate_epoch(self, epoch, mode='valid'):
        ...
        # Progress bar
        evaluate_bar = tqdm(self.data_loaders[mode], ascii=True, ncols=100, desc=f'Epoch {epoch}. {mode.title()} batches')
        ...
        if mode == 'valid':
            ...
            num_samples = self.args.num_test_samples  # num_samples = 20
        else:
        ...
        for batch_data, batch_id in evaluate_bar:
            ...
            # compute loss_mask
            loss_mask = self.net.compute_loss_mask(seq_list, self.args.obs_length).to(self.device)

            # compute metric_mask
            metric_mask = compute_metric_mask(seq_list)

            all_output, all_aux_outputs = self.net.forward(inputs, ground_truth, num_samples=1, if_test=True)

            # compute losses
            losses = self.net.compute_model_losses(all_output, ground_truth, loss_mask, inputs, all_aux_outputs)
