...
class dataset_set_name(BaseDataset):
    ...
    def __init__(self,args, set_name):
        ...
        #self.data_augmentation = args.data_augmentation if set_name == 'train' else False
    ...
    def augment_traj_and_images(self, batch_data):
        ...
        cur_ori = abs_pixel_coord.copy()
        abs_pixel_coord[:, :, 0] = cur_ori[:, :, 0] * np.cos(th) - cur_ori[:, :, 1] * np.sin(th)
        abs_pixel_coord[:, :, 1] = cur_ori[:, :, 0] * np.sin(th) + cur_ori[:, :, 1] * np.cos(th)

        # get shift value
        s = abs_pixel_coord[self.args.seq_length - 1]

        shift_value = np.repeat(s.reshape((1, -1, 2)), self.args.seq_length, 0)

        abs_pixel_coord = shift_value
        ...
        input_traj_maps = input_traj_maps.view(bs * T, old_H, old_W).permute(1, 2, 0).numpy().astype('float32')
        # # keypoints to list of tuples
        # need to clamp because some slightly exit from the image(元组列表的关键点需要进行钳制，因为有些稍微退出了图像)
        ...
        transform = A.Compose([
            ...
            A.OneOf([  # perspective or shear(透视或剪切)
                A.Perspective(
    ...
    def __getitem__(self, i):
        ...
        # rescale(缩放) coordinates wrt pixel coordinates
