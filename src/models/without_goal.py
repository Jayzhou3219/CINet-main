...
from fusion_transformer import TransformerModel


class ZJL_GOAL_Without_Goal(Base_Model):
    def __init__(self, args, device):
        ...
        self.goal_module = UNet(
            enc_chs=(self.num_image_channels + 1,
                     32, 32, 64, 64, 64),
    ...
    def forward(self, inputs, ground_truth, num_samples=1, if_test=False):
        ...
        # Groundtruth feature map for goal_loss
        # gt_template = create_gaussian_heatmap_template(size=int(1050),
        #                                                kernlen=31,
        #                                                nsig=4,
        #                                                normalize=False)
        # gt_template = torch.Tensor(gt_template).to(self.device)
        ...
        for frame_idx in range(self.args.obs_length, self.args.obs_length + self.args.pred_length):
            # gt_future = gt_traj[:, frame_idx, :].squeeze(1)
            # gt_future_map = get_patch(gt_template, gt_future.cpu().numpy(), 64, 64)
            # gt_future_map = torch.stack(gt_future_map).unsqueeze(1)  # (32, 64, 64)

            # A Heatmap for N pedenstrain
            # obs_traj_maps = inputs["input_traj_maps"][:, 0:frame_idx]
            # channel_pooling = my_channel_pooling(kernel_size=(1, frame_idx), stride=(1, frame_idx))
            # obs_traj_maps_ = channel_pooling(obs_traj_maps)

            # Concat the tensor_image as the input of UNet
            # input_goal_module = torch.cat((tensor_image, obs_traj_maps_), dim=1)  # H_S

            # Compute goal maps
            # goal_logit_map_start = self.goal_module(input_goal_module)
            # goal_prob_map = torch.sigmoid(goal_logit_map_start[:, -1:] / self.args.sampler_temperature)
            ...
            # If only sub-ZJL, set aux_outputs = {}
            aux_outputs = {}

            # list of auxiliary outputs
            # aux_outputs = {
            #     "goal_point_start": goal_point_start,
            #     "goal_logit_map": goal_logit_map_start,
            #     "gt_future_map": gt_future_map}
            ...
            # spatial_input_embedded = self.dropout_input_spatial(self.relu(
            #     self.input_embedding_layer_spatial(current_agents)))
            ...
            ############################################
            fusion_transformer = TransformerModel(temporal_input_embedded,
                                                      self.embedding_size,
                                                      self.nhead,
                                                      self.d_hidden,
                                                      self.n_layers,
            ...
            fusion_transformer_out = fusion_transformer(temporal_input_embedded, mask=False)
            ...
            #     noise_to_cat = noise.repeat(attn_output_final.shape[0], 1)  # (32, 16)
            #     fusion_feat = torch.cat((attn_output_final, noise_to_cat), dim=1)  # (32, 80)
            if self.add_noise_traj:
                # Concatenate noise to fusion output
    ...
    #     goal_logit_map = aux_outputs["goal_logit_map"]
    #
    #     goal_BCE_loss = Goal_BCE_loss(goal_logit_map, gt_future_map, loss_mask)
    #
    #     losses = {
    #         "traj_MSE_loss": MSE_loss(outputs, ground_truth, loss_mask),
    #         "goal_BCE_loss": goal_BCE_loss, }
    ...
    def compute_model_metrics(self,
        ...
        if metric_name == 'ADE':
            ...
            return FDE_best_of_goal(predictions, ground_truth,
                                     metric_mask, self.args)
