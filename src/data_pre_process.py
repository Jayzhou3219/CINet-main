...
def is_legitimate_traj(traj_df, step):
    ...
    equi_spaced = np.arange(frame_ids[0], frame_ids[-1] + 1, step, dtype=int)  # np.arange():生成一个指定起点终点和步长的列表
    # check that frame rate is evenly-spaced(检查帧速率是否均匀分布)
...
class Trajectory_Data_Pre_Process(object):
    def __init__(self,args):
        ...
        # Trajectoried and data_batches folder
        self.data_batches_path = os.path.join(self.args.save_dir, 'data_batches')
        ...
        for set_name in ['train', 'valid', 'test']:
            #batches folders
            self.batches_folders[set_name] = os.path.join(self.data_batches_path, f"{set_name}_batches")
            maybe_makedir(self.batches_folders[set_name])
            # batches confirmation file paths
            self.batches_confirmation_files[set_name] = os.path.join(self.data_batches_path,f"fineshed_{set_name}_batches.txt")
        ...
        self.experiment = create_experiment(self.args.dataset)(self.args.test_set, self.args)
        print("Done.\n")
    ...
    def create_data_batches(self,set_name):
        """
        Create data baches for the DataLoader object
        """
