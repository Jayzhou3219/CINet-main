...
sys.path.append('.')  # needed to launch from .
sys.path.append('..')  # needed to launch from ./scripts

from src.utils import str2bool

def get_parser():
    parser = argparse.ArgumentParser(description='Human-Trajectory-Forecasting')
    
    # Dataset, Test set and model
    parser.add_argument('--dataset', '-d', default='sdd', type=str,
    ...
    parser.add_argument(
        '--validate_every', '-ve', default=100, type=int,
    ...
    parser.add_argument(
        '--device', default="cuda:1", type=str, help='Which device to use')
    parser.add_argument(
    ...
    # General parameters
    parser.add_argument(
        '--down_factor', default=8, type=int, help="Image down scale factor for CNN")
    parser.add_argument(
        '--traj_normalization', default=1, type=int,
    ...
    parser.add_argument(
        '--fast_debug', '-fd', default=True, type=str2bool, const=True,
        nargs='?', help="Set to True for fast debug mode")
    parser.add_argument(
        '--fast_debug_num', default=10, type=int,
        help="Number of batches in fast debug mode")
    parser.add_argument(
        '--compute_valid_nll', default=False, type=str2bool, const=True,
        nargs='?',
        help="Set to True to compute validation KDE-NLL(Kernel Density Estimate-based Negative Log Likelihood)")
    parser.add_argument(
...
def main_parser():
    """
    pipeline from pardes args to args
    """
    # Parse input parameters
    parser = get_parser()
    parsed_args = parser.parse_args()
    parsed_args = check_and_add_additional_args(parsed_args)
    # Config files are created and stored at the first run only; if args yaml does not exist, save it!
...
def check_and_add_additional_args(args):
    ...
    # set directories
    args.base_dir = '/home/zjl9/code/Human-Trajectory-Forecasting/output'  # base directory
    args.save_base_dir = '/home/zjl9/code/Human-Trajectory-Forecasting/output/output_normal'  # for saving output and models
